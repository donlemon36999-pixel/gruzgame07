...
import {
  GRUZGAME07_CHECKIN_PRICE_ETH,
  getGruzGame07ContractAddress,
...
const STORAGE_KEY = "gruzgame07:players";
...
export default function Home() {
  ...
  const name = useMemo(
    () => context?.user?.displayName || (address ? `Командир ${address.slice(2, 6).toUpperCase()}` : "Командир"),
  ...
  const handleSyncTaps = async () => {
    ...
    try {
      ...
      const data = withGruzGame07BuilderCodeDataSuffix(
        encodeFunctionData({
          abi: gruzGame07OnchainAbi,
          functionName: "tap",
  ...
  const handleCheckin = async () => {
    ...
    try {
      ...
      await sendTransactionAsync({
        to: contractAddress,
        data,
        value: parseEther(GRUZGAME07_CHECKIN_PRICE_ETH),
        chainId: base.id,
      });
    } catch (err) {
      pendingActionRef.current = null;
  ...
  return (
    <main className={styles.container}>
      <div className={styles.swampLayer} />
      <div className={styles.reedsLayer} />
      <div className={styles.mudLayer} />
      <div className={styles.bunkerLayer} />
      <div className={styles.crocSilhouette} aria-hidden>
        🐊
      </div>
      ...
      <section className={styles.card}>
        ...
        <p className={styles.hint}>Бойцов с check-in: {checkedUsersCount}</p>

        {view === "menu" && (
          <div className={styles.menuButtons}>
            <button className={styles.swampButton} onClick={() => setView("leaderboard")} type="button">
              Лидерборд
            </button>
            <button className={styles.swampButton} onClick={() => setView("checkin")} type="button">
              Ончейн чек-ин
            </button>
            <button className={styles.swampButton} onClick={() => setView("tap")} type="button">
              К танку!
            </button>
          </div>
        )}
          ...
          <div className={styles.viewBlock}>
            ...
            >
              <div className={styles.tankVisual}>
                <span className={styles.tankHitFlash}>🐊</span>
                <div className={styles.tankHull}><span className={styles.crocOnTank}>🐊</span></div>
                <span className={`${styles.tankTrack} ${styles.tankTrackLeft}`} />
                <span className={`${styles.tankTrack} ${styles.tankTrackRight}`} />
              </div>
              <span className={styles.tankCaption}>ТАПАЙ ТАНК</span>
            </button>
            <p className={styles.hint}>Базовый тап = 1 очко. Каждый check-in добавляет +10% к тапу.</p>
            <p className={styles.hint}>Неотправленных ударов: {pendingTaps}</p>
            <button
              className={styles.swampButton}
              type="button"
            ...
            >
              {isSubmittingTap || isWritePending || isTxMining
                ? "Транзакция..."
                : `Отправить ${pendingTaps} удар(ов) onchain`}
            </button>
          </div>
        )}
        ...
        {view === "checkin" && (
          <div className={styles.viewBlock}>
            ...
            <p className={styles.hint}>Стоимость check-in: {GRUZGAME07_CHECKIN_PRICE_ETH} ETH</p>
          ...
          <div className={styles.viewBlock}>
            ...
            <div className={styles.leaderboard}>
              {leaderboard.length === 0 ? (
                <p className={styles.hint}>Пока нет бойцов. Станьте первым!</p>
              ) : (
                leaderboard.map((row) => (
